...
rowsgene = which(data_mutations_filtered$V1 == "PBRM1")
...
```{r}
annot_col = data.frame(cluster)
row.names(annot_col) = substr(rownames(cluster), 1, 12)

sampleDistMatrix = as.matrix( sampleDists )
rownames(sampleDistMatrix) = colnames(RNAseq_filteredToEdit)
colnames(sampleDistMatrix) = colnames(RNAseq_filteredToEdit)
