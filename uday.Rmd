...
# Assuming you've performed K-means clustering and have cluster assignments in `patient_data`

# Perform PCA for dimensionality reduction
pca_result <- prcomp(data_clinical_patient_filtered, scale. = FALSE)  # You might want to use scaled data

# Extract the first two principal components
pca_data <- as.data.frame(pca_result$x[, 1:2])

# Add cluster assignments to the PCA data
pca_data$cluster <- patient_data$cluster
...
ggplot(pca_data, aes(PC1, PC2, color = factor(cluster))) +
  geom_point() +
  labs(title = "K-means Clustering of Patient Data (PCA)", x = "Principal Component 1", y = "Principal Component 2") +
